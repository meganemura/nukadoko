...
| 黙って状態を変える `Then` | `mutates` は step ごとの宣言であり、**実行が観測した内容と照合され**、Then の位置にある step が書き込みを行えば宣言にかかわらず失敗します |

最後の項目は正確に言う価値があります。
宣言だけでは無価値になってしまうからです。
ツールは、実行が自分自身の request context と page を通じて実際に行った非 GET な呼び出しを数え、約束にではなく計測に対して失敗させます。
