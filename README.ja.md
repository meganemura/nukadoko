...
上でスコープ外とした retry の形とは違います。
それまでは、run の中のあらゆる scenario が 1 つずつ順番に実行されるため、フル run の壁時計はスイートの大きさとともに伸びます。
CI ランナーが並列でどれだけカバーできるかは関係ありません。
だからこそ、上で述べた分け方、あらゆる PR に `nuka check` を、merge・deploy・夜間の build に `nuka run` を、という分け方は、単なる便利さの話ではなくなります。
スイートが大きくなるほど、その分け方は PR ゲートを速いまま保つために必須になります。
