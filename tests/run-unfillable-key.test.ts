...
// exactly this: `receipt: null`, the same "never began" shape src/run/run-
...
describe("nuka run: unfillable required args key guard", () => {
  ...
  it("fails the violating scenario before its step ever runs, with no receipt", async () => {
  ...
  it("every other scenario in the same feature file still runs normally, receipts and all", async () => {
    const stdout = createCaptureSink();
    const stderr = createCaptureSink();
    ...
    expect(byLine.get(3).steps[0].receipt).toBeNull();
