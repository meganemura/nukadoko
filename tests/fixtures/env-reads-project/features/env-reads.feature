Feature: ctx.requireEnv records which names were required
  ...
  Scenario: two requireEnv calls, one of them repeated, land on the receipt deduplicated and in read order
  ...
  Scenario: a step that requires a missing env var still reports the name on its failed receipt
