...
// Reconcile's own one-step-behind limit: `freshField` does not exist as an
// accessor yet when this step's own body creates it, so this write is not
// measured —
...
// The #private integration test itself: must not crash, and must return the
// real value, proving `this` inside `revealSecret()` is the literal
// instance, never a wrapper.
