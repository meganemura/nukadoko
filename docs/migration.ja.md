...
- **`setParallelCanAssign` を値として使う**: `nukadoko/compat` はこれを export していません。
  ...
  nukadoko には今日の時点で並列実行がないため、work-assignment のコールバックが実際に制御すべき対象がそもそも存在しません。
  ...
  roadmap の M12 が `nuka run --concurrency <n>` を加えます。これは feature ファイルを丸ごと worker プロセスへ渡す仕組みです。
  それが入ったとき、`setParallelCanAssign` はその配り方が実際に何を制御すべきかに照らして再検討されます。
  ただし、この名前が戻ってくるとはここでは約束していません。
  どの scenario をどの scenario の隣に置いてよいかを決めるコールバックは、worker へ渡されたファイルとは別の仕組みであり、再検討が同じ拒否に終わることもあります。
