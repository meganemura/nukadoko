...
// Both `.nukadoko` trees are cleared before anything runs, because this
// script is not idempotent without it. `export/allure-results/` is
// appended to across runs by design (docs/spec.md "The state directory":
// it is safe to delete whenever a fresh launch is wanted), so a second
// invocation against a warm workspace finds the first one's results still
// sitting there and the report assertions count them: "Total 7" reads
// back as "Total 10", in a scenario that has nothing to do with what
// broke. That cost two separate debugging sessions before it was written
// down. Everything removed here is regenerated by the run that follows.
for (const dir of [
  path.join(suiteDir, ".nukadoko"),
  path.join(suiteDir, "fixture-project", ".nukadoko"),
  path.join(suiteDir, "fixture-project", "playwright-suite", ".nukadoko"),
]) {
